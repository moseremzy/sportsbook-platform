...
<div class="container" :style="interactive_store.container_css">
    ...
    <div class="sub_container" :style="interactive_store.sub_container_css">
      ...
      <HEADER page_name="add-country" searchbox_placeholder="Search countries" />
      ...
      <div class="add-item-form-container">
        <form class="add-item-form grid-form">
          ...
          <!-- Submit -->
          <div class="form-group grid-full">
            <button type="submit" @click.prevent="submitCountry" class="submit-button">
              Add Country
            </button>
          </div>
...
function validate() {
  ...
  if (!country_info.slug.trim()) {
    ...
    country_info_error.slug_err = ''
  }

  const file = flagImage.value?.files?.[0]
  if (!file) {
    country_info_error.flag_err = 'Please upload a flag image'
    valid = false
...
async function submitCountry() {
  ...
  formData.append('country_image', file, file.name)

  interactive_store.toggle_loading_overlay(true)

  try {

    const res = await API.add_country(formData)
...
function resetForm() {
  ...
  if (flagImage.value) flagImage.value.value = ''
}
</script>
